...
func partition1(A []int, low, high int) (q int) {
...
//第二种方式类似于选择排序，分为未处理区域和已处理区域，找到小于pivot的就插入已处理区域
func partition(A []int, low, high int) (q int) {
	pivot := A[high]
	//i维护已处理区域的边界后一位，j遍历未处理区域的边界
	i := low
	for j := low; j < high; j++ {
		//遇见一个小于pivot的就在已处理区域扩充一个
		if A[j] < pivot {
			A[j], A[i] = A[i], A[j]
			i++
		}
	}
	//此时索引i再未处理边界上，也就是大于pivot的的数字
